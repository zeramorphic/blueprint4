...
impl BlockStack {
    ...
    pub fn push(&mut self, span: SourceSpan, block: TokenBlock) {
        self.stack.last_mut().unwrap().contents.push((span, block))
    }

    pub fn finalise(mut self) -> Result<Vec<(SourceSpan, TokenBlock)>, ParseError> {
        if self.stack.len() > 1 {
    ...
    match block {
        ...
        TokenBlock::Environment {
            ...
            if math_mode_environments.contains(&environment.as_str()) {
                TokenBlock::Mathematics {
                    display: true,
                    contents: TokenBlock::Environment {
                        begin_span,
                        end_span,
                        environment,
                        body,
